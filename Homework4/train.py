...
def args_parser():
    parser = argparse.ArgumentParser(
        description="Train script for Surface Reconstruction Network"
    )
    parser.add_argument(
        "--data_root", type=str, required=True, help="Path to the training dataset"
    )
    parser.add_argument(
        "--sample_size", type=int, default=25000, help="Batch size for training"
    )
    parser.add_argument(
        "--num_iters", type=int, default=100000, help="Number of iteraions to train"
    )
    parser.add_argument(
        "--checkpoint_iters",
        type=int,
    ...
    parser.add_argument(
        "--lambda_sdf", type=float, default=1.0, help="Weight for the sdf loss term"
    )
    parser.add_argument(
        "--lambda_gradient",
        type=float,
        default=1.0,
        help="Weight for the gradient loss term",
    )
    parser.add_argument(
        "--lambda_eikonal",
        type=float,
        default=0.1,
        help="Weight for the Eikonal loss term",
    )

    parser.add_argument(
        "--use_fourier", action="store_true", help="Use Fourier features"
    )
    parser.add_argument(
        "--fourier_mapping_size",
        type=int,
        default=64,
        help="Mapping size for Fourier features",
    )
    parser.add_argument(
        "--fourier_scale", type=float, default=5.0, help="Scale for Fourier features"
    )

    parser.add_argument(
        "--mix_dataset",
        action="store_true",
        help="Use mixed dataset with both point cloud and SDF samples",
    )
...
def compute_gradient(points, pred_sdf):
    grad_outputs = torch.ones_like(
        pred_sdf, requires_grad=False, device=pred_sdf.device
    )
...
def train(args):
    ...
    if args.mix_dataset:
        dataset = RecTrainDataset_Mixed(
            args.data_root, sample_size=args.sample_size, ratio=0.2
        )
    ...
    for obj_idx in range(len(dataloader.dataset)):
        ...
        print(
            f"Processing object {obj_idx + 1}/{len(dataloader.dataset)}, filename is {object_name}"
        )
        ...
        with tqdm(
            range(args.num_iters), desc=f"Training {object_name}", unit="iter"
        ) as pbar:
            for iter_idx in pbar:
                ...
                loss = (
                    args.lambda_sdf * loss_sdf
                    + args.lambda_gradient * loss_grad
                    + args.lambda_eikonal * eikonal_loss
                )
                ...
                if (iter_idx + 1) % args.checkpoint_iters == 0:
                    checkpoint_path = os.path.join(
                        log_path, f"model_epoch_{iter_idx + 1}.pth"
                    )
