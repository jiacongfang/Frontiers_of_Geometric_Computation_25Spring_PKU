...
def train_vqvae(args):
    ...
    loss_config = {"codebook_weight": 1.0}
    ...
    print(
        f"Model parameters: {sum(p.numel() for p in vqvae.parameters() if p.requires_grad):,}"
    )
    ...
    for epoch in range(num_epochs):
        ...
        for batch_idx, data in enumerate(pbar):
            ...
            writer.add_scalar(
                "Loss/Train/Total", log_dict["loss_total"].item(), global_step
            )
            writer.add_scalar(
                "Loss/Train/Reconstruction", log_dict["loss_rec"].item(), global_step
            )
            writer.add_scalar(
                "Loss/Train/Codebook", log_dict["loss_codebook"].item(), global_step
            )
            writer.add_scalar(
                "Loss/Train/NLL", log_dict["loss_nll"].item(), global_step
            )
